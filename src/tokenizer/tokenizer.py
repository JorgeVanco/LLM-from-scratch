    ...
    def encode(self, text: str) -> list[int]:
        if self.special_tokens:
            splitted_text = re.split(
                "(" + "|".join(re.escape(t) for t in self.special_tokens) + ")", text
            )
        else:
            splitted_text = [text]
        ...
        for split_text in splitted_text:
            if split_text not in self.special_tokens:
                ...
                for pretoken in pretokenized_text:
                    pretoken = tuple(
                        bytes([c]) for c in pretoken.group().encode("utf-8")
                    )
                    if len(pretoken) >= 2:
...
def load_tokenizer(
    tokenizer_dir: str, special_tokens: list[str] | None = None
) -> Tokenizer:
